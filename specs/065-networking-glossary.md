...
- [x] `GlossaryEntry` gains three optional fields: `say` (ONE interview-ready
...
- [x] `docs/content-pipeline.md` §1 documents the speakable-field contract
      alongside the existing def contract
- [x] Glossary gains a **Networking** group; new entries with all three
      speakable fields populated: `tcp`, `udp`, `handshake`, `headofline`,
      `grpc`, `protobuf`, `webrtc`, `lasteventid`
- [x] Existing strays move into the Networking group with keys unchanged
      (<Term> refs keep working): `dns`, `tls`, `websocket`, `sse`, `polling`
- [x] Schema test: every entry in the Networking group has non-empty
      `say`/`reachFor`/`trap`; `say` is a single sentence (no more than one
      terminal period)
- [x] Reference screen renders the new group and (if present) the speakable
