...
- [ ] `src/content/edges.ts` (new): central typed list, one row per edge:
      ...
      graph.
- [ ] Briefing page renders **WHERE THIS TOUCHES** at the foot (replacing the
...
- [ ] The ~54 existing `related.sections` links migrate to edges using the
...
- [ ] The four networking edges from §C (to `api-design`,
...
- [ ] Schema tests: every edge's endpoints resolve to section ids; every
...
- [ ] Fill-out spec checklist note added to content-pipeline §7: a briefing
      spec ships its outbound edges (both directions phrased) in the same
      change as its blocks and deck
- [ ] verify.sh green; screenshots of a briefing foot reviewed (desktop +
      380px)
