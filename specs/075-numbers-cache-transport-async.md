...
- [ ] Every id in coverage-map §B.4 bundle **N3** (cache and delivery, 11
      ids) exists with a 3-step `derivation`, `boundingPhysics`, `confusions`
- [ ] Every id in bundle **N4** (transport, transfer, and async, 20 ids)
      likewise
- [ ] Derivation discipline per §B.4 — in this bundle that especially means
...
- [ ] Reuse over re-adding where a source value already resolves (§B.4 note);
      unique ids; existing schema tests green
- [ ] verify.sh green; no screenshot changes expected (data only)
