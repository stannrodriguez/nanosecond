...
export interface ComponentEntry {
  ...
  termKey: string
  /**
   * The Forge (spec 070): the component id in content/forge.ts that gates this
   * part, or null for parts you always have (product-spec §3.8 — the unlock
   * graph covers six parts; the front door and the app tier are not among them).
   */
  forge: string | null
}
...
export const COMPONENTS: ComponentEntry[] = [
  ...
  },
  {
    ...
    ch: 'net',
    cost: 300,
    capacity: '1M msgs/s intake — the buffer, not the drain',
    when: 'Writes are bursty and a short delay is acceptable.',
    risk: '$300. Risk: it buys time, not capacity — steady overload just grows the backlog.',
  ...
  },
  {
    ...
    ch: 'net',
    cost: 100,
    capacity: '5k writes/s drained per worker',
    when: 'Always, alongside a queue — the drain is what makes the buffer finite.',
    risk: '$100 each. Risk: size the drain below sustained intake and the backlog never comes back.',
    simplifies: 'Every worker is identical and always healthy; real fleets have stragglers, poison messages that stall a consumer, and warmup before full drain rate.',
    termKey: 'worker',
    forge: 'workers',
  },
  {
    ...
    ch: 'net',
    cost: 450,
    capacity: '~45% of reads served at the edge, never reaching your stack',
    when: 'Reads are cacheable and your users are far from your servers.',
    risk: '$450/mo. Risk: invalidation — the edge will happily serve yesterday for as long as you told it to.',
    simplifies: 'One flat hit fraction; real edge hit rates swing with cache-control headers, personalization, and how many PoPs your traffic actually lands on.',
    termKey: 'cdn',
    forge: 'cdn',
  },
]
